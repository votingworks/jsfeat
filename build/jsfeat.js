(function (global, factory) {
    ...
    var jsfeat_imgproc = /*#__PURE__*/Object.freeze({
        ...
        grayscale: grayscale,
        resample: resample,
        box_blur_gray: box_blur_gray,
        gaussian_blur: gaussian_blur,
        hough_transform: hough_transform,
        pyrdown: pyrdown,
        scharr_derivatives: scharr_derivatives,
    ...
    var jsfeat_linalg = /*#__PURE__*/Object.freeze({
        ...
        svd_decompose: svd_decompose,
        svd_solve: svd_solve,
        svd_invert: svd_invert,
        eigenVV: eigenVV
    });
    ...
    const lmeds = function(params, kernel, from, to, count, model, mask, max_iters) {
        ...
        var err = err_buff.f32;
        var min_median = 1000000000.0, sigma=0.0, median$1=0.0;
        ...
        for (; iter < niters; ++iter) {
            ...
            median$1 = median(err, 0, count-1);

            if(median$1 < min_median) {
                min_median = median$1;
                M.copy_to(model);
                result = true;
            }
    ...
    var motion_estimator = /*#__PURE__*/Object.freeze({
        __proto__: null,
        ransac: ransac,
        lmeds: lmeds
    });
    ...
    */
    ...
    var jsfeat_haar = /*#__PURE__*/Object.freeze({
        ...
        detect_multi_scale: detect_multi_scale,
        group_rectangles: group_rectangles
    });
    ...
    const build_pyramid = function(src, min_width, min_height, newInterval) {
        ...
        next = (interval + 1)|0;
        scale_to = (Math.log(Math.min(sw / min_width, sh / min_height)) / Math.log(scale))|0;

        var pyr_l = ((scale_to + next * 2) * 4) | 0;
        ...
        for (i = next; i < scale_to + next * 2; ++i) {
            src1 = img_pyr.data[(i << 2) - (next << 2)];
            src0 = img_pyr.data[i<<2];
            nw = src1.cols >> 1;
            nh = src1.rows >> 1;
            if(new_pyr || nw != src0.cols || nh != src0.rows) {
        ...
        for (i = next * 2; i < scale_to + next * 2; ++i) {
            src1 = img_pyr.data[(i << 2) - (next << 2)];
            nw = src1.cols >> 1;
            nh = src1.rows >> 1;
    ...
    var jsfeat_bbf = /*#__PURE__*/Object.freeze({
        ...
        get scale_to () { return scale_to; },
        prepare_cascade: prepare_cascade,
        build_pyramid: build_pyramid,
        detect: detect$3,
        group_rectangles: group_rectangles$1
    });
